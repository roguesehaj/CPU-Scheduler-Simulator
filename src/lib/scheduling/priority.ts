...
export function priorityNonPreemptive(processes: Process[]): SchedulingResult {
  ...
  while (done.size < processes.length) {
    const available = processes.filter(p => !done.has(p.id) && p.arrivalTime <= currentTime);

    if (available.length === 0) {
      const next = processes.filter(p => !done.has(p.id)).sort((a, b) => a.arrivalTime - b.arrivalTime)[0];
    ...
    gantt.push({ processId: p.id, start: currentTime, end });
    currentTime = end;
    done.add(p.id);
...
export function priorityPreemptive(processes: Process[]): SchedulingResult {
  ...
  while (currentTime < maxTime) {
    if ([...remainingBurst.values()].every(v => v === 0)) break;

    const available = processes.filter(p => p.arrivalTime <= currentTime && remainingBurst.get(p.id)! > 0);

    if (available.length === 0) {
      const next = processes.filter(p => remainingBurst.get(p.id)! > 0).sort((a, b) => a.arrivalTime - b.arrivalTime)[0];
      if (!next) break;
      gantt.push({ processId: null, start: currentTime, end: next.arrivalTime });
      currentTime = next.arrivalTime;
      continue;
    ...
    available.sort((a, b) => a.priority - b.priority || remainingBurst.get(a.id)! - remainingBurst.get(b.id)! || a.arrivalTime - b.arrivalTime || a.id.localeCompare(b.id));
